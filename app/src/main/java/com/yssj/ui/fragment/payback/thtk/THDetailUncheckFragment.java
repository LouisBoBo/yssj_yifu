...
public class THDetailUncheckFragment extends BaseFragment {
	
	private TextView tvTitle_base;
	private LinearLayout img_back;
	private ImageView img_right_icon;
	
	private TextView tv_order_je,tv_order_code,tv_apply_date,tv_end_time,tv_apply_time,tv_th_reason;
	private Button btn_payback_cancle;
	...
	public View initView() {
		view = View.inflate(context, R.layout.activity_payback_th_detail_uncheck, null);
		...
		tv_th_reason = (TextView) view.findViewById(R.id.tv_th_reason);	// 退货原因
	...
	public void initData() {
		...
		if(bundle != null){
			...
			if(returnShop != null){
				...
				tv_apply_date.setText("申请退货时间:" + DateFormatUtils.format(returnShop.getAdd_time(), "yyyy-MM-dd HH:mm:ss"));
				new CountDownTimer(returnShop.getLast_time().getTime()-System.currentTimeMillis(),1000) {
					
					@Override
					public void onTick(long millisUntilFinished) {
						
						tv_end_time.setText("商家在" + DateUtil.FormatMilliseondToEndTime(millisUntilFinished) + "内未处理，将自动同意。");
					}
					
					@Override
					public void onFinish() {
						tv_end_time.setText("商家在指定时间内没有处理，系统已自动同意退货。");
					}
				}.start();
