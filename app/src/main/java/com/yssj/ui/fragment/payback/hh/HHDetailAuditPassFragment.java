 ...
 *
 */
@SuppressLint("NewApi") public class HHDetailAuditPassFragment extends BaseFragment {
	...
	private EditText et_wl_name,et_wl_num,et_receive_address;
	...
	private RadioGroup rg_select_address;
	private View customView;
	
	private String address_id = "0";   // 地址号
	private String express_id = "" ;	// 物流单号
	private RadioButton rbt_old_address;
	private RadioButton rbt_other_address;
	

	@Override
	public View initView() {
		view = View.inflate(context, R.layout.activity_payback_hh_detail_auditpass, null);
		...
		et_receive_address = (EditText) view.findViewById(R.id.et_receive_address);	// 收货地址
		et_receive_address.setOnClickListener(this);
		
			
		btn_submit = (Button) view.findViewById(R.id.btn_submit);	// 提交按钮
		btn_submit.setOnClickListener(this);
		
		customView = View.inflate(context, R.layout.payback_hh_select_address, null);	// 自定义选择地址view
	...
	public void initData() {
		...
		if(bundle != null){
			...
			if(returnShop != null){
				...
				tv_apply_date.setText("换货时间:" + DateFormatUtils.format(returnShop.getAdd_time(), "yyyy-MM-dd HH:mm:ss"));
				new CountDownTimer(returnShop.getLast_time().getTime()-System.currentTimeMillis(),1000) {
					
					@Override
					public void onTick(long millisUntilFinished) {
						
						tv_end_time.setText("请您在" + DateUtil.FormatMilliseondToEndTime(millisUntilFinished) + "内填写，逾期将自动取消换货。");
					}
	...
	public void onClick(View v) {
		...
		case R.id.et_receive_address:	// 下拉选择物流地址
			showSelectAddressDown();
			break;
			
		case R.id.rbt_old_address:
			address_id = "0" ;
			et_receive_address.setText("原地址返回");
			popupWindow2.dismiss();
			break;
		case R.id.rbt_other_address:
			intent = new Intent(context, ManMyDeliverAddr.class);
	...
	private void showSelectWlNameDown(){
		...
		if(popupWindow == null){
			popupWindow = new PopupWindow(listView, et_wl_name.getWidth(),350);
		}
	...
	private void showSelectAddressDown(){
		
		rg_select_address = (RadioGroup) customView.findViewById(R.id.rg_select_address);
		
		rbt_old_address = (RadioButton) customView.findViewById(R.id.rbt_old_address);
		rbt_old_address.setOnClickListener(this);
		
		rbt_other_address = (RadioButton) customView.findViewById(R.id.rbt_other_address);
		rbt_other_address.setOnClickListener(this);
		
		
		popupWindow2 = new PopupWindow(customView, et_receive_address.getWidth(),274);
		...
		popupWindow2.showAsDropDown(et_receive_address,0,0);
	}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode == 101 && resultCode == 102){
			...
			et_receive_address.setText(address);
		}
	}
...
//		String receiveRaddress = et_receive_address.getText().toString();
		...
		new SAsyncTask<Void, Void, ReturnInfo>((FragmentActivity)context, R.string.wait){
			...
			protected ReturnInfo doInBackground(FragmentActivity context,
					Void... params) throws Exception {
				return ComModel.addLogistics(context, returnShop.getId()+"", address_id, express_id);
			}
