...
//拼团夺宝---我的参与记录
public class PinTuanDuoItemFragment extends Fragment {
    ...
    public static PinTuanDuoItemFragment newInstances(int position, Context context) {
        PinTuanDuoItemFragment instance = new PinTuanDuoItemFragment();
    ...
    protected void initData(final String index) {
        ...
        new SAsyncTask<String, Void, List<HashMap<String, Object>>>(getActivity(), 0) {
            ...
            protected List<HashMap<String, Object>> doInBackground(FragmentActivity context, String... params)
                    throws Exception {
                return ModQingfeng.SnatchJoin(context, index); // TODO
            }
            ...
            protected void onPostExecute(FragmentActivity context, List<HashMap<String, Object>> result, Exception e) {
                ...
                if (result != null) {
                    if (PinTuanDuoItemFragment.this.index == 1) {
                        pList.clear();
                    }
    ...
    private class DateAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                convertView = LayoutInflater.from(context).inflate(R.layout.item_pintuan_duobaojilu, null);
...
//            DuoBaoJiLu_user user = (DuoBaoJiLu_user) mapObj.get("user");
            ...
            final String in_rollUserName = mapObj.get("in_rollUserName") + "";
            holder.tv_name.setText(in_rollUserName+"的团"); // 获奖者名称


            final String etime = mapObj.get("etime") + ""; // 开奖时间 (揭晓时间前半分)
...
//                holder.tv_tuikuan.setVisibility(View.GONE);
            ...
            if (status.equals("2")) { //2已经开奖,   分为中奖和未中奖

                String in_userId = mapObj.get("in_userId").toString();

                if (!in_userId.equals(YCache.getCacheUser(context).getUser_id() + "")) { // user里面拿出来的userid不对
                    //未中奖
...
//                    holder.tv_tuikuan.setVisibility(View.VISIBLE);
                    ...
                    holder.tv_time.setText(
                            DateFormatUtils.format(Long.parseLong(mapObj.get("etime").toString()), "yyyy.MM.dd HH:mm:ss"));
...
//                    holder.tv_tuikuan.setVisibility(View.VISIBLE);
                    ...
                    holder.tv_yjx.setVisibility(View.GONE); //已揭晓图标
                    holder.rl_bootom.setVisibility(View.VISIBLE);
                    holder.tv_time.setText(
                            DateFormatUtils.format(Long.parseLong(mapObj.get("etime").toString()), "yyyy.MM.dd HH:mm:ss"));
                    holder.tv_time_answer.setText("揭晓时间");
            ...
            holder.tv_tuikuan.setVisibility(View.GONE);//没有退款功能了
...
//            int num3 = num1 + num2;
            ...
            try {
                num = Integer.parseInt(mapObj.get("num").toString()) + Integer.parseInt(mapObj.get("v_num").toString());
            } catch (Exception e) {
                e.printStackTrace();
            }


            holder.tv_count.setText(" " + num); // 参与次数
