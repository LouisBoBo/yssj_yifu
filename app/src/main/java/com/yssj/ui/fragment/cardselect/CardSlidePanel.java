...
//    private View bottomLayout; // 卡片下边的三个按钮布局
    ...
    private int itemMarginTop = 3; // 卡片距离顶部的偏移量
    ...
    private int isShowing; // 当前正在显示的小项
...
//                    vanishOnBtnClick(type);
    ...
    public void bottomClick(View v, int w) {
        // 点击的是bottomLayout里面的一些按钮
        btnLock = true;
        if (w == 0 || w == 1) {//0：左边的不喜欢按钮  1：右边的喜欢按钮
            int type = -1;
            if (w == 0) {
                type = VANISH_TYPE_LEFT;
            } else if (w == 1) {
...
//    }
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            LogYiFu.e("onViewReleased", "xvel:" + xvel + "      yvel:" + yvel);
            cardSwitchListener.onCenterImgHide(releasedChild, xvel, yvel);
            animToSide((CardItemView) releasedChild, (int) xvel, (int) yvel);
        }

        @Override
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            LogYiFu.e("onViewReleased", "dx:" + dx + "      left:" + left);
            cardSwitchListener.onCenterImgShow(child, left, dx);
            return left;
        }

        @Override
        public int clampViewPositionVertical(View child, int top, int dy) {
            LogYiFu.e("onViewReleased", "dy:" + dy + "      top:" + top);
            return top;
        }
    }
    ...
    private void animToSide(CardItemView changedView, int xvel, int yvel) {
        ...
        if (finalX == initCenterViewX) {
            ...
            cardSwitchListener.onBackCenter();
        } else {
            // 2. 向两边消失的动画
...
//        }
     ...
     */
    public void fillData(List<CardDataItem> dataList, int indext) {
        this.dataList = dataList;
        isShowing = indext;
        int num = viewList.size();
        if (dataList.size() - indext < 4) {//上一件的时候 最后三件点击上一件 实际itme小于给定的viewList.size();
            num = dataList.size() - indext;
        }
        for (int i = 0; i < num; i++) {
            CardItemView itemView = viewList.get(i);
            itemView.fillData(dataList.get(i + indext));
            itemView.setVisibility(View.VISIBLE);
            itemView.getImageCenterIv().setVisibility(View.GONE);
        }

        if (null != cardSwitchListener) {
            cardSwitchListener.onShow(indext);
        }
    }
    ...
    public interface CardSwitchListener {
        ...
        public void onBackCenter();//没有消失 回到中间
    }
}