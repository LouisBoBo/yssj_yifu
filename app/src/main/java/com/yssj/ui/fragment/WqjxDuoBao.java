...
public class WqjxDuoBao extends Fragment {
    ...
    private class DateAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            holder.tv_duoshaoqi.setText(issue_code); // 期号

            final String otime = mapObj.get("otime").toString();
            holder.tv_years
                    .setText(DateFormatUtils.format(Long.parseLong(mapObj.get("otime").toString()), "yyyy.MM.dd")); // 揭晓时间年月日
            holder.tv_sfm.setText(DateFormatUtils.format(Long.parseLong(mapObj.get("otime").toString()), "HH:mm:ss")); // 揭晓时间时分秒

            final String in_name = mapObj.get("in_name").toString();
            holder.name_of_winner.setText(in_name); // 获奖者昵称
            ...
            holder.tv_join_num.setText(num3 + ""); // 本期参与人数
...
//			SetImageLoader.initImageLoader(context, holder.img_head, (String) mapObj.get("in_head"), "");
            ...
            final String in_head = mapObj.get("in_head").toString(); // 用户头像
