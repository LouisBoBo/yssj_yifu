...
@SuppressLint("NewApi")
public class HHFragment extends BackHandledFragment implements
		onDeteleImgLintener {
	...
	private EditText et_hh_exlpain;
	private Button btn_submit_apply;
	private TextView et_apply_service, et_hh_reason;
	...
	public View initView() {
		...
		et_hh_reason = (TextView) view.findViewById(R.id.et_hh_reason); // 换货原因
		et_hh_reason.setOnClickListener(this);

		et_hh_exlpain = (EditText) view.findViewById(R.id.et_hh_exlpain); // 换货说明
	...
	public void initData() {
		...
		/*
		 * Bundle bundle = getArguments(); if(bundle != null){ order_code =
		 * bundle.getString("order_code"); // 获取订单号 order_price =
		 * bundle.getString("order_price"); }
		 */
		if (!isHH) {
...
//		listhh.add("七天无理由退换货");
	...
	private void showSelectHHDown() {
		...
		if (popupWindow == null) {
			popupWindow = new PopupWindow(listView, et_hh_reason.getWidth(),
	...
	public void onClick(View v) {
		...
		case R.id.et_apply_service: // 下拉列表选择申请服务
			...
			break;
		case R.id.et_hh_reason: // 下拉列表选择换货原因
			showSelectHHDown();
			break;
	...
	private void submitApply() {
		...
		if (explain.length() > 200) {
			ToastUtil.showLongText(context, "对不起，换货说明不能超过200字");
			return;
		}
