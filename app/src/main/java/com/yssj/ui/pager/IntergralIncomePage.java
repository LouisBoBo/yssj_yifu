...
public class IntergralIncomePage extends BasePager {
	private HashMap<String, Object> map;
//	private  boolean isTiXianMingXi;//是否是提现额度明细

	public IntergralIncomePage(Context context) {
		super(context);
	...
	public void initData() {
		...
		if(IntergralDetailListFragment.isTiXianMingXi){
			tv_no_join.setText("暂无提现额度新增明细");
		}else{
			tv_no_join.setText("暂无积分收入明细");
		}
		
		currPage = 1;
		queryIntergralList();
		mAdapter = new MyIntergralPageAdapter(context,0,false);
	...
	private void queryIntergralList() {
		new SAsyncTask<Integer, Void, List<HashMap<String, Object>>>(
			...
			protected List<HashMap<String, Object>> doInBackground(FragmentActivity context,
				...
				if(IntergralDetailListFragment.isTiXianMingXi){  //提现额度明细  ---收入的
					return ComModel2.getTixianedumingxi(context, currPage,"1","desc","id");
				}else{  //积分明细  --收入的
