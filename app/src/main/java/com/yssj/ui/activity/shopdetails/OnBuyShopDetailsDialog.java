 ...
 *
 */
public class OnBuyShopDetailsDialog extends Dialog implements OnClickListener, OnItemClickListener {
...
//	private ImageLoadingListener animateFirstListener;
    ...
    public OnBuyShopDetailsDialog(Context context, int style, int width, int height, Shop shop
        ...
        if (colorId != -1 && sizeId != -1) {
            ...
            for (int i = 0; i < listsTypes.size(); i++) {
                ...
                if (sType.getColor_size().equals(color_size)) {
                    ...
                    try {
                        groupPrice = Double.parseDouble(shop.getAssmble_price());
                    } catch (Exception e) {
                        e.printStackTrace();
                    }

                    tv_price.setText("¥" + new DecimalFormat("#0.0").format(groupPrice));
