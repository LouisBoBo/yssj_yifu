...
public class FolderTextView extends TextView {  
    ...
    // 默认展开文字  
    private static final String DEFAULT_UNFOLD_TEXT = "查看全文";
    // 默认固定行数  
    private static final int DEFAULT_FOLD_LINE = 4;
    // 默认收起和展开文字颜色  
    private static final int DEFAULT_TAIL_TEXT_COLOR = Color.parseColor("#FF3F8B");
    // 默认是否可以再次收起  
...
//    };
        ...
        if (!mIsFold) {  
            ...
            if (line < layout.getLineCount()) {  
                ...
                if (index > 0) {  
                    ...
                    LogYiFu.d(TAG, "strWhichHasExactlyFoldLine-->" + strWhichHasExactlyFoldLine);  
    ...
    protected void onDraw(Canvas canvas) {  
    	LogYiFu.e(TAG, "onDraw() " + mCountOnDraw++ + ", getMeasuredHeight() " + getMeasuredHeight());  
...
//    }
    ...
    private void resetText() {  
        ...
        if (layout.getLineCount() <= getFoldLine()) {
        	if(!title.isEmpty()){//穿搭标题
        		SpannableString spanStr = new SpannableString(mFullText); 
            	spanStr.setSpan(new ForegroundColorSpan(Color.parseColor("#FF3F8B")), 0, title.length(),
...
//    }
    ...
    private SpannableString createFoldSpan(String text) {  
        ...
        LogYiFu.e(TAG, (System.currentTimeMillis() - startTime) + "ms");  
        ...
        spanStr.setSpan(new ForegroundColorSpan(mTailColor), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        return spanStr;  
    }  
    ...
    private String tailorTextBackUp(String text) {
    	LogYiFu.e(TAG, "使用备用方法: tailorTextBackUp() " + mCountBackUp++);
    ...
    private String tailorText(String text) {
        ...
        while (find != 0 && end > start) {
        	LogYiFu.e(TAG, "使用二分法: tailorText() " + mCountBinary++);
            if (find > 0) {
                end = mid - 1;
            } else if (find < 0) {
                start = mid + 1;
