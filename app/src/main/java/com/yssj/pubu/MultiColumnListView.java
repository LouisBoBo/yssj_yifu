...
public class MultiColumnListView extends PLA_ListView {
	...
	private void init(AttributeSet attrs) {
		...
		if (attrs == null) {
			...
			// mColumnPaddingLeft =
			// a.getDimensionPixelSize(R.styleable.PinterestLikeAdapterView_plaColumnPaddingLeft,
			// 0);
			// mColumnPaddingRight =
			// a.getDimensionPixelSize(R.styleable.PinterestLikeAdapterView_plaColumnPaddingRight,
			// 0);
			a.recycle();
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int jianju3;// 左右两边加上每列之间的间距 --- 穿搭列表左右和中间的都是5dp
		jianju3 = (getColumnNumber() + 1) * 5;
		columnWidth = (getMeasuredWidth() - mListPadding.left - mListPadding.right - mColumnPaddingLeft
				- mColumnPaddingRight - DP2SPUtil.dp2px(getContext(), jianju3)) / getColumnNumber();

		for (int index = 0; index < getColumnNumber(); ++index) {
			// index：列数序号 0开始
			mColumns[index].mColumnWidth = columnWidth;

			mColumns[index].mColumnLeft = mListPadding.left + (index + 1) * DP2SPUtil.dp2px(getContext(), 5)
