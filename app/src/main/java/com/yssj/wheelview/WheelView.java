...
public class WheelView extends View {
	...
	private static final int VALUE_TEXT_COLOR = 0xFF222222;

	/** Items text color */
	private static final int ITEMS_TEXT_COLOR = 0xFF989898;
...
//		setBackgroundResource(R.drawable.wheel_bg);
	 ...
	 */
	private HashMap<String, String> getTextItem(int index) {
		if (adapter == null || adapter.getItemsCount() == 0) {
			return null;
		}
	...
	private String buildText(boolean useCurrentValue) {
		...
		for (int i = currentItem - addItems; i <= currentItem + addItems; i++) {
			if (useCurrentValue || i != currentItem) {
				HashMap<String, String> map = getTextItem(i);
				if(null != map){
				String text = map.get("AreaName");
				if (text != null) {
	...
	private int getMaxTextLength() {
		...
		for (int i = Math.max(currentItem - addItems, 0); i < Math.min(
			...
			String text = adapter.getItem(i).get("AreaName");
			if (text != null
	...
	public String getCurrentItemValue() {
		...
		return getAdapter().getItem(getCurrentItem()).get("AreaName");
	}
